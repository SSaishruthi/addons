...
class F1Score(Metric):
    ...
    def result(self):
        ...
        # calculate precision
        precision_macro = tf.math.divide_no_nan(self.true_positives, p_sum)

        r_sum = tf.cast(self.true_positives + self.false_negatives, tf.float32)
        # calculate recall
        recall_macro = tf.math.divide_no_nan(self.true_positives, r_sum)
        ...
        if self.average == 'weighted':
            f1_int_weights = tf.cast(
                tf.math.divide_no_nan(
                    ...
                    tf.reduce_sum(self.weights_intermediate)), tf.float32)
